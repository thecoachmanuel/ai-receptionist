...
export type AIProvider = "elevenlabs" | "gemini" | "vapi";
...
export async function updateElevenLabsSettings(data: {
  ...
  if (data.vapiPublicKey !== undefined) {
    $set.vapiPublicKey = data.vapiPublicKey.trim();
  }
  if (data.vapiPrivateKey !== undefined) {
    $set.vapiPrivateKey = data.vapiPrivateKey.trim();
  }
  if (data.vapiAssistantId !== undefined) {
    $set.vapiAssistantId = data.vapiAssistantId.trim();
  }
